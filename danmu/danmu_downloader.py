...
class DanmuDownloader:
    ...
    def clear_cache(self) -> None:
        """清空弹幕客户端的所有缓存"""
        self.danmu_client.clear_cache()
        logger.info("已清空弹幕下载器缓存")
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict: 缓存统计信息
        """
        return self.danmu_client.get_cache_stats()
    ...
    def _search_anime(self, series_name: str, season: Optional[int] = None) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            logger.debug(f"搜索动漫: {series_name}, 季数: {season}")
            ...
            from_cache = library_result.get('from_cache', False)
            cache_info = "(来自缓存)" if from_cache else "(来自API)"
            logger.debug(f"获取到 {len(animes)} 个作品 {cache_info}")
            ...
            for anime in animes:
                ...
                if series_name.lower() in anime_title.lower() or anime_title.lower() in series_name.lower():
                    ...
                    logger.debug(
                        f"找到匹配动漫: {anime_title}, 第{anime_season}季, ID: {anime.get('animeId')}")
            ...
            if season is not None:
                for anime in matched_animes:
                    if anime.get('season', 1) == season:
                        logger.debug(f"精确匹配到第{season}季: {anime.get('title')}")
                        return anime
            ...
            logger.debug(
                f"选择动漫: {selected_anime.get('title')}, 第{selected_anime.get('season', 1)}季")
    ...
    def _get_episodes(self, anime_id):
        ...
        try:
            # 尝试从缓存中获取作品标题
            anime_title = f"作品{anime_id}"
            if hasattr(self.danmu_client, '_library_cache') and self.danmu_client._library_cache:
                for anime in self.danmu_client._library_cache.get('data', []):
                    if anime.get('animeId') == anime_id:
                        anime_title = anime.get('title', f'作品{anime_id}')
                        break

            logger.debug(f"获取分集信息: {anime_title} (ID: {anime_id})")

            # 获取弹幕源列表（使用缓存）
            sources_result = self.danmu_client.get_anime_sources(
                anime_id, use_cache=self.use_cache)
            if not sources_result.get('success'):
                logger.warning(f"获取弹幕源失败: {anime_title} (ID: {anime_id})")
                return None

            sources = sources_result.get('sources', [])
            from_cache = sources_result.get('from_cache', False)
            cache_info = "(来自缓存)" if from_cache else "(来自API)"
            logger.debug(f"获取到 {len(sources)} 个弹幕源 {cache_info}")

            if not sources:
                logger.warning(f"作品没有可用的弹幕源: {anime_title} (ID: {anime_id})")
                return None
            ...
            for source in sources:
                ...
                logger.debug(f"获取弹幕源: {provider_name} (ID: {source_id})")

                # 获取该源的分集列表（使用缓存）
                episodes_result = self.danmu_client.get_source_episodes(
                    source_id, use_cache=self.use_cache)
                if not episodes_result.get('success'):
                    media_id = source.get('mediaId', '')
                    logger.warning(
                        f"获取分集列表失败: {provider_name}({media_id}, ID: {source_id})")
                    continue

                episodes = episodes_result.get('episodes', [])
                from_cache = episodes_result.get('from_cache', False)
                cache_info = "(来自缓存)" if from_cache else "(来自API)"
                ...
                if episodes:
                    ...
                    logger.debug(
                        f"{provider_name} 找到 {len(episodes)} 集 {cache_info}")

            if all_episodes:
                logger.debug(f"总共获取到 {len(all_episodes)} 个弹幕源的数据")
                return all_episodes
            else:
                logger.warning(
                    f"作品没有获取到任何有效的分集数据: {anime_title} (ID: {anime_id})")
                return None

        except Exception as e:
            logger.error(f"获取分集信息时出错: {anime_title} (ID: {anime_id}), 错误: {e}")
            return None
    ...
    def _download_danmu(self, episode_id):
        ...
        """
        try:
            logger.debug(f"下载弹幕: {episode_id}")

            # 使用新版API下载弹幕
            danmu_result = self.danmu_client.get_episode_danmaku(
                str(episode_id))
            ...
            if danmu_data:
                ...
                logger.debug(f"下载到 {comment_count} 条弹幕")
    ...
    def _save_danmu_xml(self, danmu_data, output_filepath, provider_name):
        ...
        """
        try:
            logger.debug(f"保存弹幕XML: {output_filepath}")
            ...
            # 提取弹幕列表
            comments = danmu_data.get('comments', []) if isinstance(
                danmu_data, dict) else danmu_data
            ...
            if success:
                logger.debug(f"XML文件保存成功: {output_filepath}")
                return True
            else:
