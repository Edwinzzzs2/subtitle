class SubtitleWatcher {
  ...
  bindEvents() {
    ...
    this.clearProcessedBtn.addEventListener("click", () =>
    ...
    this.cronSchedule.addEventListener("input", () =>
  ...
  async loadConfig() {
    try {
      ...
      // 保存当前定时任务状态
      const wasCronRunning =
        this.cronEnabled && this.cronEnabled.checked && this.cronJob;
      ...
      if (result.success && result.config) {
        ...
        // 加载弹幕API配置
        this.danmuApiUrl.value =
          (config.danmu_api && config.danmu_api.base_url) || "";
        this.danmuApiToken.value =
          (config.danmu_api && config.danmu_api.token) || "";

        // 如果之前定时任务在运行或者配置中启用了定时任务，则重新启动
        if (
          (wasCronRunning || this.cronEnabled.checked) &&
          this.cronSchedule.value.trim()
        ) {
  ...
  async clearCache() {
    ...
    this.setButtonLoading(this.clearCacheBtn, true);

    try {
      const response = await this.apiCall("/clear-cache", {
        method: "POST",
      });

      if (response && response.success) {
        this.showNotification("弹幕缓存已清除", "success");
      } else {
        this.showNotification(
          response ? response.message : "清除缓存失败",
          "error"
        );
      }
    } catch (error) {
  ...
  updateStatus(status) {
    ...
    if (this.statusText && this.statusDot) {
      ...
      this.statusDot.className = isRunning
        ? "status-dot online"
        : "status-dot offline";
    }
  ...
  updateLogs(logs) {
    ...
    const filteredLogs = logs.filter((log) => {
      ...
      return (
        message.toLowerCase().includes(filterValue) ||
        timestamp.toLowerCase().includes(filterValue)
      );
    });
  ...
  calculateNextExecution(cronExpression) {
    try {
      ...
      while (attempts < maxAttempts) {
        if (
          this.matchesCronField(next.getSeconds(), second) &&
  ...
  updateCronStatus(text, enabled) {
    if (this.cronStatus) {
      ...
      this.cronStatus.className = enabled
        ? "status-text enabled"
