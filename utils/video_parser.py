...
class VideoFileParser:
    ...
    def parse_video_filename(self, filepath):
        ...
        try:
            ...
            # 获取文件目录和扩展名
            file_dir = os.path.dirname(os.path.abspath(filepath)) if os.path.dirname(filepath) else os.getcwd()
            ...
            # 提取剧名
            series_name = self._extract_series_name(filename_without_ext, episode_info)
            if not series_name:
    ...
    def _extract_series_name(self, filename, episode_info):
        ...
        if episode_info.get('season'):
            # 移除 S01E14 格式
            clean_name = re.sub(r'[Ss]\d+[Ee]\d+', '', clean_name, flags=re.IGNORECASE)
        ...
        if not clean_name or len(clean_name) < 2:
            ...
            clean_name = re.sub(r'[Ss]\d+[Ee]\d+', '', clean_name, flags=re.IGNORECASE)
            clean_name = re.sub(r'[Ee][Pp]\s*\d+', '', clean_name, flags=re.IGNORECASE)
            clean_name = re.sub(r'[-_\s]+', ' ', clean_name)
            clean_name = clean_name.strip(' -_')
    ...
    def get_danmu_filepath(self, video_info, danmu_source='IqiyiID'):
        ...
        """
        try:
            danmu_filename = self.generate_danmu_filename(video_info, danmu_source)
            if not danmu_filename:
                return None
            
            danmu_filepath = os.path.join(video_info['file_dir'], danmu_filename)
